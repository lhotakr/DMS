...
public partial class UnitsView : UserControl
{
    ...
    private void RefreshBaseUnitChoices()
    {
        if (ListDimensions.SelectedItem is not UnitDimension dimension)
        {
            CmbBaseUnit.ItemsSource = Array.Empty<string>();
            CmbBaseUnit.SelectedItem = null;
            return;
        }
        ...
        CmbBaseUnit.ItemsSource = codes;

        if (string.IsNullOrWhiteSpace(dimension.BaseUnitCode))
        {
            // Existing data can predate the BaseUnitCode editor.
            // Prefer the active default unit when it is also a valid
            // conversion identity. Otherwise use the single identity unit.
            var suggested = choices
                .FirstOrDefault(x =>
                    x.IsDefault &&
                    x.ScaleToBase == 1m &&
                    x.OffsetToBase == 0m);
            ...
            if (suggested is null)
            {
                ...
                if (identityUnits.Count == 1)
                {
                    suggested = identityUnits[0];
                }
            }

            if (suggested is not null)
            {
                dimension.BaseUnitCode = suggested.Code;

                TxtStatus.Text = string.Format(
                    T("SYS01.Units.Status.BaseUnitSuggested"),
                    dimension.Code,
                    suggested.Code);
            }
        }

        CmbBaseUnit.SelectedItem = codes.FirstOrDefault(code =>
            string.Equals(
                code,
    ...
    private void BaseUnitChanged(
        object sender,
        SelectionChangedEventArgs e)
    {
        if (ListDimensions.SelectedItem is not UnitDimension dimension ||
            CmbBaseUnit.SelectedItem is not string code)
        {
            return;
        }

        dimension.BaseUnitCode = code;
    }
