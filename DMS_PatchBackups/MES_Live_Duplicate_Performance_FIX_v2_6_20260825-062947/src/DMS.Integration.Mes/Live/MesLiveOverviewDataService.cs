        ...
        var sql = $"""
            ...
            (
                SELECT
                    m.WorkcenterID,
                    m.ShiftID,
                    SUM(COALESCE(m.PerformanceTotal, 0)) AS TotalAmount,
                    SUM(COALESCE(m.DurationUtilization, 0)) AS UtilizationSeconds
                FROM [{_schema}].[FactMdaMes] m
                INNER JOIN CurrentShiftKeys sk
                    ON sk.WorkcenterID = m.WorkcenterID
                   AND sk.ShiftID = m.ShiftID
            ...
            SELECT TOP (@maxRows)
                ...
                CASE
                    WHEN sp.UtilizationSeconds IS NOT NULL
                         AND sp.UtilizationSeconds > 0
                         AND sp.TotalAmount IS NOT NULL
                    THEN CAST((sp.TotalAmount * 60.0) / sp.UtilizationSeconds AS decimal(18, 3))
            ...
            LEFT JOIN ShiftPerformance sp ON sp.WorkcenterID = m.WorkcenterID AND sp.ShiftID = m.ShiftID
            WHERE NULLIF(LTRIM(RTRIM(w.Code)), '') IS NOT NULL
              {workcenterPredicate}
            ORDER BY
                COALESCE(
