...
public sealed class MesWorkcenterDashboardSnapshot
{
    ...
    public MesWorkcenterShiftMetrics ShiftMetrics { get; init; } = new();
    ...
    public decimal? ExpectedByNow
    {
        get
        {
            var plannedPerformance =
                ShiftMetrics.PlannedPerformance
                ?? PlannedPerformance;

            if (!plannedPerformance.HasValue
                || plannedPerformance.Value <= 0m
                || !ShiftStart.HasValue
                || !ShiftEnd.HasValue
            ...
            return plannedPerformance.Value
                   * (decimal)(now - ShiftStart.Value).TotalMinutes;
        }
    }
}


public sealed class MesWorkcenterShiftMetrics
{
    ...
    public double ActualPerformance =>
        AvailableSeconds > 0d
            ? (double)PerformanceTotal * 60d / AvailableSeconds
            : 0d;

    // Exact FASTEC DDP script formulas supplied for this workstation view:
    // BRUTTO = GoodPerformance (Shift) + ReworkPerformance (Shift)
    public decimal GrossAmount =>
        PerformanceGood + PerformanceRework;

    // NETTO = GoodPerformance (Shift) - BadPerformance (Shift)
    public decimal NetAmount =>
        PerformanceGood - PerformanceBad;
    ...
    public double AvailabilityOee
    {
        ...
        {
            var denominator =
                AvailableSeconds + FailureSeconds;

            return denominator > 0d
                ? AvailableSeconds / denominator * 100d
                : 0d;
        }
    }

    public double PerformanceOee =>
        PlannedPerformance.HasValue
        && PlannedPerformance.Value > 0m
            ? ActualPerformance
              / (double)PlannedPerformance.Value
              * 100d
            : 0d;
