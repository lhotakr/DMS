...
public partial class MesReportingView
{
    ...
    private void BuildOeeReportPresentation(MesReportDefinition definition)
    {
        ...
        ChartBorder.Height = 310d;
        ChartBorder.MinHeight = 260d;
        ...
        GridReport.Columns.Add(FinalTextColumn("MES06.OEE.Personnel", "Personnel", "Personnel", 165));
        ...
        var width = Math.Max(1000d, rows.Count * 82d + 110d);
    ...
    private sealed class Mes06OeeChartElement : FrameworkElement
    {
        ...
        protected override void OnRender(DrawingContext dc)
        {
            ...
            var left = 48d;
            var top = 12d;
            var bottom = _compact ? 210d : 220d;
            var right = ActualWidth - 12d;
            var maxStack = Math.Max(100d, _rows.Max(r => r.Oee + r.AvailabilityLoss + r.PerformanceLoss + r.QualityLoss));
            maxStack = Math.Max(110d, Math.Ceiling(maxStack / 10d) * 10d);
            double Y(double value) => bottom - Math.Clamp(value / maxStack,0d,1d) * (bottom-top);
            ...
            for (double p = 0d; p <= maxStack; p += 20d)
            {
                ...
                var ft=new FormattedText($"{p:0}%",CultureInfo.CurrentCulture,FlowDirection.LeftToRight,new Typeface("Segoe UI"),7.2,Brushes.Black,dpi);
                dc.DrawText(ft,new Point(left-ft.Width-5,y-ft.Height/2));
            }
            var slot=(right-left)/Math.Max(1,_rows.Count);
            var bar=Math.Min(42d,slot*0.58d);
            for(var i=0;i<_rows.Count;i++)
            {
                var r=_rows[i];
                var x=left+i*slot+(slot-bar)/2d;
                var current=0d;
                DrawStack(dc,x,bar,ref current,r.Oee,Color.FromRgb(35,230,85),Y,dpi);
                DrawStack(dc,x,bar,ref current,r.AvailabilityLoss,Color.FromRgb(255,55,50),Y,dpi);
                DrawStack(dc,x,bar,ref current,r.PerformanceLoss,Color.FromRgb(255,245,175),Y,dpi);
                DrawStack(dc,x,bar,ref current,r.QualityLoss,Color.FromRgb(55,55,235),Y,dpi);
                var label =
                    string.IsNullOrWhiteSpace(r.ShiftName)
                        ? $"{r.WorkcenterCode}|{r.Personnel}"
                        : $"{r.WorkcenterCode}|{r.ShiftName}|{r.Personnel}";
                var ft=new FormattedText(label,CultureInfo.CurrentCulture,FlowDirection.LeftToRight,new Typeface("Segoe UI"),_compact?5.8:6.5,Brushes.Black,dpi){MaxTextWidth=115,Trimming=TextTrimming.CharacterEllipsis};
                dc.PushTransform(new RotateTransform(-45,x+bar/2,bottom+7));
                dc.DrawText(ft,new Point(x+bar/2,bottom+7));
                dc.Pop();
            }
            DrawOeeLegend(dc,right-330,8,dpi,_legendLabels);
        }
        ...
        {
            if(value<=0d) return;
            var y0=y(current); current+=value; var y1=y(current);
            dc.DrawRectangle(new SolidColorBrush(color),null,new Rect(x,y1,width,Math.Max(0.8,y0-y1)));
            if(value>=3d)
            {
                var ft=new FormattedText($"{value:0.00}%",CultureInfo.CurrentCulture,FlowDirection.LeftToRight,new Typeface("Segoe UI"),6.2,Brushes.Black,dpi);
                dc.DrawText(ft,new Point(x+(width-ft.Width)/2,y1+(y0-y1-ft.Height)/2));
            }
        }

        private static void DrawOeeLegend(DrawingContext dc,double x,double y,double dpi,IReadOnlyList<string> labels)
        {
            var items=new[]{
            ...
            {
                dc.DrawRectangle(new SolidColorBrush(item.Item2),new Pen(Brushes.Gray,0.4),new Rect(x,y,10,10));
                var ft=new FormattedText(item.Item1,CultureInfo.CurrentCulture,FlowDirection.LeftToRight,new Typeface("Segoe UI"),6.6,Brushes.Black,dpi);
                dc.DrawText(ft,new Point(x+15,y-1)); y+=15;
            }
        }
    }
