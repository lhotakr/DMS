...
public sealed class MesReportingEnrichmentService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            if (!mesStart.HasValue || !mesEnd.HasValue || mesEnd <= mesStart)
            {
                continue;
            }

            var clippedMesStart = mesStart.Value < from ? from : mesStart.Value;
            var clippedMesEnd = mesEnd.Value > to ? to : mesEnd.Value;
            if (clippedMesEnd <= clippedMesStart)
            {
                continue;
            }

            var humanStart = GetDateTime(reader, "HumanStarttime") ?? clippedMesStart;
            var humanEnd = GetDateTime(reader, "HumanEndtime") ?? clippedMesEnd;
            var overlapStart = humanStart > clippedMesStart ? humanStart : clippedMesStart;
            var overlapEnd = humanEnd < clippedMesEnd ? humanEnd : clippedMesEnd;
            if (overlapEnd <= overlapStart)
            {
                continue;
            }

            var mesSeconds = (clippedMesEnd - clippedMesStart).TotalSeconds;
            var overlapSeconds = (overlapEnd - overlapStart).TotalSeconds;
            if (mesSeconds <= 0d || overlapSeconds <= 0d)
            {
                continue;
            }

            var fraction = Math.Clamp(overlapSeconds / mesSeconds, 0d, 1d);
            var workcenter = GetString(reader, "WorkcenterCode");
            var humanCode = GetString(reader, "HumanCode");
            var firstName = GetString(reader, "FirstName");
            var lastName = GetString(reader, "LastName");
            var shiftName = GetString(reader, "ShiftName");
            var shiftStarttime = GetDateTime(reader, "ShiftStarttime");
            var personKey = !string.IsNullOrWhiteSpace(humanCode)
                ? humanCode
                : $"{lastName}|{firstName}";

            // Shift is intentionally part of the aggregation key.
            // "All shifts" therefore returns one OEE row per workcenter/person/shift,
            // while a selected shift is still filtered directly in SQL.
            var key = $"{workcenter}\u001F{shiftName}\u001F{personKey}";
            ...
            var unoccupied = (double)(GetDecimal(reader, "DurationUnoccupied") ?? 0m) * fraction;
            var available = (double)(GetDecimal(reader, "DurationUtilization") ?? 0m) * fraction;
            var down = (double)(GetDecimal(reader, "DurationDown") ?? 0m) * fraction;
            var total = (double)(GetDecimal(reader, "PerformanceTotal") ?? 0m) * fraction;
            var good = (double)(GetDecimal(reader, "PerformanceGood") ?? 0m) * fraction;
            var bad = (double)(GetDecimal(reader, "PerformanceBad") ?? 0m) * fraction;
            var rework = (double)(GetDecimal(reader, "PerformanceRework") ?? 0m) * fraction;

            accumulator.PlannedShutdownSeconds += unoccupied;
            accumulator.AvailableSeconds += available;
            accumulator.FailureSeconds += down;
            accumulator.TotalAmount += total;
            accumulator.GoodUnits += good;
            accumulator.BadUnits += bad;
            accumulator.ReworkUnits += rework;
            ...
            if (planned.HasValue && planned.Value > 0m && available > 0d)
            {
                accumulator.PlannedPerformanceWeighted += (double)planned.Value * available;
                accumulator.PlannedPerformanceWeight += available;
            }
        }
        ...
        return accumulators.Values
            ...
            .ThenBy(row => row.Personnel, StringComparer.CurrentCultureIgnoreCase)
            .ToList();
    }
...
public sealed class Mes06OeeReportRecord
{
    ...
    public static Mes06OeeReportRecord FromAccumulator(Mes06OeeAccumulator a)
    {
        ...
        var personnel = string.Join(", ",
            new[] { a.LastName, a.FirstName }
                .Where(value => !string.IsNullOrWhiteSpace(value)));
        if (string.IsNullOrWhiteSpace(personnel))
        {
            personnel = !string.IsNullOrWhiteSpace(a.HumanCode) ? a.HumanCode : "—";
        }
        else if (!string.IsNullOrWhiteSpace(a.HumanCode))
        {
            personnel += $" [{a.HumanCode}]";
        }
