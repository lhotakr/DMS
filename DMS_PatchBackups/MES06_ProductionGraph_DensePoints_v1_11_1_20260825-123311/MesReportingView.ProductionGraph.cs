...
public partial class MesReportingView
{
        ...
        DateTime to)
    {
        return rows
            ...
            .Select(group =>
            {
                ...
                var utilization =
                    first.DurationUtilizationSeconds;

                var performance =
                    first.PerformanceTotal;

                if (!utilization.HasValue
                    || utilization.Value <= 0m
                    || !performance.HasValue
                    || performance.Value < 0m)
                {
                    return null;
                }
    ...
    private static IReadOnlyList<Mes06ProductionRatePoint> BuildHalfHourRatePoints(
