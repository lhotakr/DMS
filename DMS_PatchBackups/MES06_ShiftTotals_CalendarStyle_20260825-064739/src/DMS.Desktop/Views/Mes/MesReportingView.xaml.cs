    ...
    private async Task LoadCurrentReportAsync()
    {
        ...
        try
        {
            ...
            _logger.AdminAction(
                ...
                "LoadMesReport",
                _user,
                $"Report={definition.Code}; From={filter.From:O}; To={filter.To:O}; Workcenter={filter.WorkcenterCode}; Order={filter.OrderCode}; Product={filter.ProductCode}; Rows={_currentRows.Count}");
        }
        catch (Exception ex)
        {
            _logger.Error(
