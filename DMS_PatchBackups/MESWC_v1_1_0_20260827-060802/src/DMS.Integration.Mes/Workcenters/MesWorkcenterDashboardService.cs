...
public sealed class MesWorkcenterDashboardService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var reportTo = now < shiftEnd.Value ? now : shiftEnd.Value;
            if (reportTo > shiftStart.Value)
            {
                var oeeTask = _reportingService.GetOeeReportAsync(
                    shiftStart.Value,
                    reportTo,
                    new[] { code },
                    shiftName,
                ...
                var graphTask = _reportingService.GetProductionGraphRowsAsync(
                    shiftStart.Value,
                    reportTo,
                    code,
                    string.Empty,
                ...
                {
                    _stateColors = await _reportingService.GetStateColorsAsync(cancellationToken);
                }

                await Task.WhenAll(oeeTask, graphTask);
                oee = oeeTask.Result.FirstOrDefault();
                graphRows = graphTask.Result;
                stateSummary = BuildStateSummary(code, graphRows, _stateColors);
            }
        }
