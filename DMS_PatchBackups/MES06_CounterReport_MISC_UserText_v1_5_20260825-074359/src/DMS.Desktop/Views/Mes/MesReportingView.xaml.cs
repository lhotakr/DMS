        ...
        IEnumerable<MesReportDefinition> definitions)
    {
        foreach (var definition in definitions)
        {
                    ...
                    definition))
            {
                definition.Name =
                    T(
                        "MES06.Report.ProductionData.Name",
                        "Production data");

                definition.Description =
                    T(
                        "MES06.Report.ProductionData.Description",
                        "FASTEC production data grouped by shifts.");

                if (definition.Chart is not null)
                {
                    definition.Chart.Title =
                        T(
                            "MES06.Report.ProductionData.ChartTitle",
                            "Production data");
                }
            }
        }
    ...
    private async Task LoadCurrentReportAsync()
    {
        ...
        try
        {
            ...
            _logger.AdminAction(
                ...
                "LoadMesReport",
                _user,
                $"Report={definition.Code}; From={filter.From:O}; To={filter.To:O}; Workcenters={GetSelectedWorkcenterAuditText()}; Shift={(CmbShift.SelectedItem as Mes06FilterChoice)?.Code}; Order={filter.OrderCode}; Product={filter.ProductCode}; Operation={TxtOperation.Text?.Trim()}; Rows={_currentRows.Count}");
        }
        catch (Exception ex)
        {
            _logger.Error(
