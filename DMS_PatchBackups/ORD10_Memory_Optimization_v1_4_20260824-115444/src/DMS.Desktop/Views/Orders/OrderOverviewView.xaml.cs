...
public partial class OrderOverviewView : UserControl
{
    ...
    private void ApplyClientFilter()
    {
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            query =
                query.Where(
                    item =>
                        ContainsIgnoreCase(item.OrderCode, search)
                        || ContainsIgnoreCase(item.ProductCode, search)
                        || ContainsIgnoreCase(item.ProductDescription, search)
                        || ContainsIgnoreCase(item.SapArticleNumber, search));
        }
        ...
        TxtStatus.Text =
            string.Format(
                T(
                    "ORD10.Status.Loaded",
                    "Načteno {0} nearchivovaných zakázek."),
                result.Count);
        ...
        long selectionVersion)
    {
        ...
        try
        {
            ...
            // Do not cancel an already running SQL command. It may finish normally,
            // but its result is accepted only if this selection is still current.
            var rows =
                await _service.GetOperationsAsync(
                    order.Id,
                    order.OrderCode);

            if (selectionVersion != _operationSelectionVersion)
            {
                return;
            }
        ...
        {
            _logger.Error(
                $"ORD10 operation load failed. Order={order.OrderCode}; SelectionVersion={selectionVersion}",
                ex);
