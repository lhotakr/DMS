...
/// Deliberately does not use FactMdaCounter because counter reset/correction semantics
/// still need per-counter validation.
/// </summary>
public sealed class MesLiveOverviewDataService
{
    ...
    public async Task<IReadOnlyList<MesShiftRecord>> GetShiftsAsync(
        DateTime from,
        DateTime to,
        CancellationToken cancellationToken = default)
    {
        var sql = $"""
            SELECT
                ...
                s.Endtime,
                s.Name
            FROM [{_schema}].[DimShiftEvent] s
            WHERE s.Endtime > @from
              AND s.Starttime < @to
        ...
        var result = new List<MesShiftRecord>();
        while (await reader.ReadAsync(cancellationToken))
        {
            result.Add(new MesShiftRecord
            {
                Id = GetGuid(reader, "ID") ?? Guid.Empty,
                DateId = GetInt(reader, "DateID") ?? 0,
                StartTime = GetDateTime(reader, "Starttime") ?? DateTime.MinValue,
                EndTime = GetDateTime(reader, "Endtime") ?? DateTime.MaxValue,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var sql = $"""
            ...
            FROM [{_schema}].[DimWorkcenter] w
            LEFT JOIN OpenMes m ON m.WorkcenterID = w.ID
            ...
            (
                ...
                INNER JOIN [{_schema}].[FactMdaMes] sm ON sm.ID = st.MesID
                INNER JOIN [{_schema}].[DimMdaState] ds ON ds.ID = st.StateID
                WHERE sm.WorkcenterID = w.ID
                  AND (st.Endtime IS NULL OR st.Endtime >= DATEADD(day, -30, GETDATE()))
            ...
            WHERE NULLIF(LTRIM(RTRIM(w.Code)), '') IS NOT NULL
              AND (@workcenter = '' OR w.Code = @workcenter)
              AND (@shiftId IS NULL OR m.ShiftID = @shiftId)
            ORDER BY w.Code;
            """;
        ...
        AddParameter(command, "@workcenter", filter.WorkcenterCode?.Trim() ?? string.Empty);
        AddGuidParameter(command, "@shiftId", filter.ShiftId);
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
