...
public partial class FrameworkWorkflowView : UserControl
{
    ...
    {
        if (WorkflowCombo.SelectedItem is not string workflowDisplay)
        {
            return;
        }
        ...
        StateCombo.SelectedItem = workflow.States
            .Select(state => state.Code)
            .FirstOrDefault(code =>
                string.Equals(
                    code,
    ...
    {
        if (WorkflowCombo.SelectedItem is not string workflowDisplay ||
            StateCombo.SelectedItem is not string stateCode)
        {
        ...
        TransitionsGrid.ItemsSource = _workflowService
            .GetAvailableTransitions(workflow, stateCode)
            .Select(transition =>
            {
