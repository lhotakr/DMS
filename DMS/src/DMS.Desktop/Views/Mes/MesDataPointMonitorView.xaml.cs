...
public partial class MesDataPointMonitorView : UserControl
{
    ...
    private bool? _lastModbusOnlineState;
    private bool? _lastNetworkReachableState;
    private string _lastConnectionStation = string.Empty;

    private EndpointProbeSnapshot _lastEndpointProbe = EndpointProbeSnapshot.Unknown;
    private DateTimeOffset _lastEndpointProbeAt;
    private string _lastEndpointProbeStation = string.Empty;
    ...
    private void TxtDeviceFilter_TextChanged(object sender, TextChangedEventArgs e)
    {
        ...
        if (CmbDevices.SelectedItem is null && _deviceView is not null)
        {
            CmbDevices.SelectedItem = _deviceView.Cast<MesDeviceEntry>().FirstOrDefault();
        }
    }
    ...
    private async Task RefreshSelectedDeviceAsync(bool forceConfigurationCheck)
    {
        ...
        try
        {
            ...
            var endpointProbeTask = ProbeEndpointAsync(
                device,
                binding,
                force: forceConfigurationCheck,
                cancellationToken: _lifetimeCancellation.Token);

            var snapshotTask = _readService.ReadSnapshotAsync(
                device,
                binding,
                _settings,
                _lifetimeCancellation.Token);

            await Task.WhenAll(endpointProbeTask, snapshotTask);

            var endpointProbe = await endpointProbeTask;
            var snapshot = await snapshotTask;

            UpdateHeader(snapshot, endpointProbe);
            UpdateRows(snapshot);
            UpdateMachineRuntime(snapshot, endpointProbe);
            LogConnectionTransition(snapshot, endpointProbe);
            UpdateStatus(snapshot, endpointProbe);
        }
    ...
    private void ReloadDeviceSelector()
    {
        ...
        try
        {
            _deviceView = CollectionViewSource.GetDefaultView(_machines);
            _deviceView.Filter = FilterDevice;
            CmbDevices.ItemsSource = _deviceView;
    ...
    private void UpdateHeader(
        MesDeviceSnapshot snapshot,
        EndpointProbeSnapshot endpointProbe)
    {
        ...
        TxtNetwork.Text = endpointProbe.PingReachable switch
        {
            true when endpointProbe.PingRoundtripMs.HasValue =>
                TF("MESDPM.Network.OnlineMs", "Online ({0} ms)", endpointProbe.PingRoundtripMs.Value),
            true => T("MESDPM.Network.Online", "Online"),
            false => T("MESDPM.Network.NoReply", "No ping reply"),
            _ => T("MESDPM.Connection.Unknown", "Unknown")
        };

        TxtConnection.Text = BuildModbusConnectionText(snapshot, endpointProbe);
        TxtLastRead.Text = snapshot.ReadAt.ToString("dd.MM.yyyy HH:mm:ss.fff");
    }

    private string BuildModbusConnectionText(
        MesDeviceSnapshot snapshot,
        EndpointProbeSnapshot endpointProbe)
    {
        if (snapshot.IsOnline)
        {
            return T("MESDPM.Connection.DataOnline", "Data online");
        }

        if (snapshot.Binding is null)
        {
            return T("MESDPM.Connection.NotConfigured", "Not configured");
        }

        if (!snapshot.Binding.Enabled)
        {
            return T("MESDPM.Connection.Disabled", "Disabled");
        }
        ...
        if (!string.Equals(
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            return T("MESDPM.Connection.Unsupported", "Deferred / unsupported");
        }

        var mappingMissing = snapshot.DataPoints.Any(point =>
            point.Quality == MesDataPointQuality.ConfigurationError);

        if (mappingMissing)
        {
            return endpointProbe.ModbusPortOpen == true
                ? T("MESDPM.Connection.PortOnlineMappingMissing", "Port online / mapping missing")
                : T("MESDPM.Connection.NotConfigured", "Not configured");
        }

        return endpointProbe.ModbusPortOpen switch
        {
            true => T("MESDPM.Connection.PortOnlineReadFailed", "Port online / read failed"),
            false => T("MESDPM.Connection.PortOffline", "Port offline"),
            _ => T("MESDPM.Connection.Offline", "Offline")
        };
    }
    ...
    private void ClearHeader()
    {
        TxtStation.Text = "—";
        TxtIpAddress.Text = "—";
        TxtNetwork.Text = T("MESDPM.Connection.Unknown", "Unknown");
        TxtDriver.Text = "—";
        TxtController.Text = "—";
    ...
    private void UpdateMachineRuntime(
        MesDeviceSnapshot snapshot,
        EndpointProbeSnapshot endpointProbe)
    {
        ...
        // Network reachability, Modbus transport and machine runtime are three
        // different states. A successful ping never proves that process data can
        // be read, and a failed Modbus read must not be presented as a stopped or
        // physically offline machine.
        if (!snapshot.IsOnline)
        {
            TxtCycleTime.Text = "—";
            ...
            if (snapshot.Binding is null ||
                snapshot.DataPoints.Any(point =>
                    point.Quality == MesDataPointQuality.ConfigurationError))
            {
                TxtMachineState.Text = T(
                    "MESDPM.MachineState.NotConfigured",
                    "Cannot determine - mapping missing");
            }
            else if (endpointProbe.PingReachable == true)
            {
                TxtMachineState.Text = T(
                    "MESDPM.MachineState.ModbusUnavailable",
                    "Cannot determine - Modbus unavailable");
            }
            else
            {
                TxtMachineState.Text = T(
                    "MESDPM.MachineState.Unavailable",
                    "Cannot determine");
            }
        ...
        if (goodCounter is null)
        {
            TxtMachineState.Text = T(
                "MESDPM.MachineState.CounterUnavailable",
                "Cannot determine - good-piece counter unavailable");
            TxtCycleTime.Text = "—";
            return;
        }
        ...
        var result = _runtimeEvaluator.Update(
            stationCode: snapshot.Device.StationCode,
            isOnline: true,
            counterValue: goodCounter.RawValue,
            readAt: snapshot.ReadAt,
            stopTimeout: TimeSpan.FromSeconds(stopTimeoutSeconds));
    ...
    private void ResetRuntimeEvaluation()
    {
        ...
        _lastModbusOnlineState = null;
        _lastNetworkReachableState = null;
        _lastConnectionStation = string.Empty;
        _lastEndpointProbe = EndpointProbeSnapshot.Unknown;
        _lastEndpointProbeAt = DateTimeOffset.MinValue;
        _lastEndpointProbeStation = string.Empty;
    }
        ...
        EndpointProbeSnapshot endpointProbe)
    {
        var stationChanged = !string.Equals(
            _lastConnectionStation,
            snapshot.Device.StationCode,
            StringComparison.OrdinalIgnoreCase);

        if (stationChanged)
        {
            _lastModbusOnlineState = snapshot.IsOnline;
            _lastNetworkReachableState = endpointProbe.PingReachable;
            _lastConnectionStation = snapshot.Device.StationCode;
            return;
        }

        if (_lastNetworkReachableState != endpointProbe.PingReachable)
        {
            _lastNetworkReachableState = endpointProbe.PingReachable;

            if (endpointProbe.PingReachable == true)
            {
                _logger?.Info(
                    $"MESDPM network online; Station={snapshot.Device.StationCode}; " +
                    $"IP={snapshot.Device.IpAddress}; User={_currentUserName}");
            }
            else if (endpointProbe.PingReachable == false)
            {
                _logger?.Warning(
                    $"MESDPM network ping unavailable; Station={snapshot.Device.StationCode}; " +
                    $"IP={snapshot.Device.IpAddress}; Error={endpointProbe.PingError}; User={_currentUserName}");
            }
        }

        if (_lastModbusOnlineState == snapshot.IsOnline)
        {
            return;
        }
        ...
        {
            _logger?.Info(
                $"MESDPM Modbus data online; Station={snapshot.Device.StationCode}; " +
                $"IP={snapshot.Device.IpAddress}; User={_currentUserName}");
        }
        else
        {
            _logger?.Warning(
                $"MESDPM Modbus data offline; Station={snapshot.Device.StationCode}; " +
                $"IP={snapshot.Device.IpAddress}; PortOpen={endpointProbe.ModbusPortOpen}; " +
        ...
        EndpointProbeSnapshot endpointProbe)
    {
        ...
        var networkText = endpointProbe.PingReachable switch
        {
            true when endpointProbe.PingRoundtripMs.HasValue =>
                $"Ping online ({endpointProbe.PingRoundtripMs.Value} ms)",
            true => "Ping online",
            false => $"Ping unavailable ({endpointProbe.PingError})",
            _ => "Ping unknown"
        };

        var portText = endpointProbe.ModbusPortOpen switch
        {
            true => $"TCP {endpointProbe.Port} open",
            false => $"TCP {endpointProbe.Port} closed/unreachable",
            _ => $"TCP {endpointProbe.Port} not tested"
        };

        TxtStatus.Text =
            $"{networkText} | {portText} | {LocalizeSnapshotStatus(snapshot)} | " +
    ...
    {
        if (item is not MesDeviceEntry device)
        {
            return false;
        }

        var filter = TxtDeviceFilter.Text?.Trim();
        ...
        return Contains(device.StationCode, filter) ||
               Contains(device.Name, filter) ||
               Contains(device.IpAddress, filter) ||
               Contains(device.DeviceType, filter) ||
               Contains(device.SuggestedDriver, filter);
    }
    ...
    private async Task<EndpointProbeSnapshot> ProbeEndpointAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var port = binding?.Port > 0 ? binding.Port : 502;

        if (!force &&
            string.Equals(
                _lastEndpointProbeStation,
                stationCode,
                StringComparison.OrdinalIgnoreCase) &&
            now - _lastEndpointProbeAt < TimeSpan.FromSeconds(5))
        {
            return _lastEndpointProbe;
        }

        var timeoutMs = Math.Clamp(
            binding?.TimeoutMs > 0
                ? binding.TimeoutMs
                : _settings.DefaultTimeoutMs,
            250,
            2000);

        var shouldTestModbusPort = string.Equals(
            binding?.Driver ?? device.SuggestedDriver,
            MesDriverKeys.BrX20ModbusTcp,
            StringComparison.OrdinalIgnoreCase);

        var pingTask = ProbePingAsync(
            device.IpAddress,
            timeoutMs,
            cancellationToken);

        var portTask = shouldTestModbusPort
            ? ProbeTcpPortAsync(
                device.IpAddress,
                port,
                timeoutMs,
                cancellationToken)
            : Task.FromResult(TcpProbeResult.NotTested);

        await Task.WhenAll(pingTask, portTask);

        var ping = await pingTask;
        var tcp = await portTask;

        _lastEndpointProbe = new EndpointProbeSnapshot
        {
            PingReachable = ping.Reachable,
            PingRoundtripMs = ping.RoundtripMs,
            PingError = ping.Error,
            ModbusPortOpen = tcp.Open,
            ModbusPortError = tcp.Error,
            Port = port
        };
        _lastEndpointProbeAt = now;
        _lastEndpointProbeStation = stationCode;
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var reply = await ping
                .SendPingAsync(ipAddress, timeoutMs)
                .WaitAsync(cancellationToken);

            return reply.Status == IPStatus.Success
                ? new PingProbeResult(true, reply.RoundtripTime, string.Empty)
                : new PingProbeResult(false, null, reply.Status.ToString());
        }
    ...
    private static async Task<TcpProbeResult> ProbeTcpPortAsync(
        string ipAddress,
        int port,
        int timeoutMs,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            return new TcpProbeResult(true, string.Empty);
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            return new TcpProbeResult(false, $"Timeout after {timeoutMs} ms");
        }
        catch (OperationCanceledException)
        {
            throw;
        ...
        {
            return new TcpProbeResult(false, ex.Message);
        }
    }
    ...
    private void ApplyLocalization()
    {
        ...
        LblNetwork.Text = T("MESDPM.Field.Network", "Network / ping");
        LblDriver.Text = T("MESDPM.Field.Driver", "Driver");
        LblConnection.Text = T("MESDPM.Field.Connection", "Modbus data");
        ...
        LblDeviceFilter.Text = T("MESDPM.Filter.DeviceSearch", "Find device:");
        LblDevice.Text = T("MESDPM.Filter.Device", "Device:");
        LblFilter.Text = T("MESDPM.Filter.Text", "Data points:");
    ...
    private string LocalizeValue(object? rawValue, string fallback)
    {
        ...
        {
            true => T("MESDPM.Value.On", "ON"),
            false => T("MESDPM.Value.Off", "OFF"),
            _ => fallback
        };
    }
    ...
    {
        public static EndpointProbeSnapshot Unknown { get; } = new();

        public bool? PingReachable { get; init; }

        public long? PingRoundtripMs { get; init; }

        public string PingError { get; init; } = string.Empty;

        public bool? ModbusPortOpen { get; init; }

        public string ModbusPortError { get; init; } = string.Empty;

        public int Port { get; init; } = 502;
    }

    private readonly record struct PingProbeResult(
        bool Reachable,
        long? RoundtripMs,
        string Error);

    private readonly record struct TcpProbeResult(
        bool? Open,
        string Error)
    {
        public static TcpProbeResult NotTested { get; } = new(null, string.Empty);
    }
