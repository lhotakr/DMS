...
public sealed class MesWorkcenterDashboardService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var contextOrderCode =
            context?.OrderCode
            ?? string.Empty;

        var activeOrderCode =
            !string.IsNullOrWhiteSpace(liveOrderCode)
                ? liveOrderCode
                : contextOrderCode;

        var contextMatchesLiveOrder =
            string.IsNullOrWhiteSpace(liveOrderCode)
            || string.Equals(
                contextOrderCode,
                liveOrderCode,
        ...
        // FactMdaMes can still point to the previous order after a live order change.
        var activeOperationCode =
            contextMatchesLiveOrder
                ? context?.OperationCode ?? string.Empty
                : string.Empty;
    ...
    private async Task<MesWorkcenterShiftMetrics> LoadCurrentShiftMetricsAsync(
        ...
        DateTime shiftStart,
        DateTime reportTo,
        decimal? plannedPerformance,
        CancellationToken cancellationToken)
    {
        // Confirmed FASTEC live behavior:
        // quantities are current in FactMdaMes, while open-row durations can lag.
        // machine_state_vector is therefore the live duration source.
        var sql =
            $"""
            ;WITH MesTotals AS
            (
                SELECT
                    SUM(COALESCE(mes.[PerformanceTotal], 0)) AS PerformanceTotal,
                    SUM(COALESCE(mes.[PerformanceGood], 0)) AS PerformanceGood,
                    SUM(COALESCE(mes.[PerformanceBad], 0)) AS PerformanceBad,
                    SUM(COALESCE(mes.[PerformanceRework], 0)) AS PerformanceRework
                ...
                WHERE wc.[Code] = @workcenterCode
                  ...
                  AND COALESCE(mes.[Endtime], @reportTo) > @shiftStart
                  AND
                  (
                        @shiftId IS NULL
                        OR mes.[ShiftID] = @shiftId
                  )
            ),
            StateDurations AS
            (
                SELECT
                    ...
                    (
                        ...
                        (
                            MILLISECOND,
                            CASE
                                WHEN ms.[starttime] < @shiftStart
                                THEN @shiftStart
                                ELSE ms.[starttime]
                            END,
                            CASE
                                WHEN ms.[endtime] IS NULL
                                  OR ms.[endtime] > @reportTo
                                THEN @reportTo
                                ELSE ms.[endtime]
                            END
                        ) / 1000.0
                ...
                INNER JOIN [{_analyticsSchema}].[DimWorkcenter] wc
                    ON wc.[ID] = ms.[wc_id]
                WHERE wc.[Code] = @workcenterCode
                  AND ms.[starttime] < @reportTo
                  AND COALESCE(ms.[endtime], @reportTo) > @shiftStart
                  AND
                  (
                        @shiftId IS NULL
                        OR ms.[shift_id] = @shiftId
                  )
                GROUP BY ms.[availability]
            )
            SELECT
                COALESCE(t.[PerformanceTotal], 0) AS PerformanceTotal,
                COALESCE(t.[PerformanceGood], 0) AS PerformanceGood,
                COALESCE(t.[PerformanceBad], 0) AS PerformanceBad,
                COALESCE(t.[PerformanceRework], 0) AS PerformanceRework,
                COALESCE((SELECT d.[DurationSeconds] FROM StateDurations d WHERE d.[availability] = 1), 0) AS AvailableSeconds,
                COALESCE((SELECT d.[DurationSeconds] FROM StateDurations d WHERE d.[availability] = 2), 0) AS FailureSeconds,
                COALESCE((SELECT d.[DurationSeconds] FROM StateDurations d WHERE d.[availability] = 3), 0) AS PlannedShutdownSeconds
            FROM MesTotals t;
            """;
        ...
        AddParameter(command, "@shiftStart", shiftStart);
        AddParameter(command, "@reportTo", reportTo);
        ...
        return new MesWorkcenterShiftMetrics
        {
            PlannedShutdownSeconds = (double)(GetDecimal(reader, "PlannedShutdownSeconds") ?? 0m),
            AvailableSeconds = (double)(GetDecimal(reader, "AvailableSeconds") ?? 0m),
            FailureSeconds = (double)(GetDecimal(reader, "FailureSeconds") ?? 0m),
            PerformanceTotal = GetDecimal(reader, "PerformanceTotal") ?? 0m,
            PerformanceGood = GetDecimal(reader, "PerformanceGood") ?? 0m,
            PerformanceBad = GetDecimal(reader, "PerformanceBad") ?? 0m,
            PerformanceRework = GetDecimal(reader, "PerformanceRework") ?? 0m,
            PlannedPerformance = plannedPerformance
        };
    }
        ...
        CancellationToken cancellationToken)
    {
        var sql = """
            SELECT TOP (60)
                ...
                CASE
                    WHEN perfRelation.[processing_time] IS NOT NULL
                     AND perfRelation.[processing_time] > 0
                    THEN 60.0 / perfRelation.[processing_time]
                    WHEN r.[exp_performance] IS NOT NULL
                     AND r.[exp_performance] > 0
                    THEN r.[exp_performance]
                    WHEN r.[processing_time] IS NOT NULL
            ...
            (
                SELECT TOP (1)
                    rel.[processing_time]
                FROM dbo.[d_pda_relation] rel
                WHERE rel.[order_operation_id] = r.[id]
                  AND rel.[processing_time] IS NOT NULL
                  AND rel.[processing_time] > 0
                ORDER BY
                    CASE WHEN rel.[code] = @workcenterCode THEN 0 ELSE 1 END,
                    CASE WHEN COALESCE(rel.[is_secondary], 0) = 0 THEN 0 ELSE 1 END,
                    rel.[change_id] DESC,
                    rel.[id]
            ) perfRelation
