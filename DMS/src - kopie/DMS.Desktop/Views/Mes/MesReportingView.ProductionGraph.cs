...
public partial class MesReportingView
{
        ...
        : FrameworkElement
    {
            ...
            DrawingContext dc)
        {
            ...
            double ToY(
                double rate)
            {
                // A physical production rate cannot be negative. Keep the
                // lower graph boundary locked exactly to 0 pcs/min.
                var safeRate =
                    Math.Max(
                        0d,
                        rate);
        ...
        bool compact)
    {
        ...
        var productiveAvailability =
            ResolveProductiveAvailability(
                rows);

        var productiveStateName =
            productiveAvailability.HasValue
                ? string.Empty
                : ResolveProductiveStateName(
                    rows);
    ...
    private static IReadOnlyList<Mes06ProductionRateInterval> BuildProductionRateIntervals(
        ...
        DateTime from,
        DateTime to,
        int? productiveAvailability,
        string productiveStateName)
    {
        return rows
            .Where(row =>
                IsProductiveGraphRow(
                    row,
                    productiveAvailability,
                    productiveStateName))
            .Select(row =>
    ...
    private static bool IsProductiveGraphRow(
        Mes06ProductionGraphRecord row,
        int? productiveAvailability,
        string productiveStateName)
    {
        if (productiveAvailability.HasValue)
        {
        ...
        // Last-resort compatibility path for installations where neither
        // availability nor enough state history is available.
        return true;
    }

    private static int? ResolveProductiveAvailability(
        IReadOnlyList<Mes06ProductionGraphRecord> rows)
    {
        var candidates =
            rows
                .Where(row =>
                    row.Availability.HasValue)
                .Select(row =>
        ...
        var groups =
            rows
                ...
                .GroupBy(row =>
                    row.MesId)
                .Where(group =>
                    group.First().DurationUtilizationSeconds.HasValue
                    && group.First().DurationUtilizationSeconds!.Value > 0m)
                .ToList();
                 ...
                 in candidates)
        {
                     ...
                     in groups)
            {
                ...
                var duration =
                    group
                        .Where(row =>
                            row.Availability == candidate)
                        .Sum(
                            ResolveGraphStateDurationSeconds);
    ...
    private static string ResolveProductiveStateName(
        IReadOnlyList<Mes06ProductionGraphRecord> rows)
    {
        var candidates =
            rows
    ...
    private static double ResolveGraphStateDurationSeconds(
        Mes06ProductionGraphRecord row)
    {
        if (row.StateDurationSeconds.HasValue
            && row.StateDurationSeconds.Value >= 0m)
        {
            return (double)row.StateDurationSeconds.Value;
        }
