...
public partial class MesReportingView
{
    ...
    private void QueueStateRowCellForeground(
        DataGridRow row,
        bool useBlack)
    {
        row.Dispatcher.BeginInvoke(
            ...
            new Action(
                () =>
                    ApplyStateRowCellForeground(
                        row,
                        useBlack)));
    }

    private static void ApplyStateRowCellForeground(
        DataGridRow row,
        bool useBlack)
    {
        if (!row.IsLoaded)
        ...
        foreach (var cell
                 in FindVisualChildren<DataGridCell>(
                     row))
        {
            if (useBlack)
            {
            ...
            foreach (var textBlock
                     in FindVisualChildren<TextBlock>(
                         cell))
            {
                if (useBlack)
                {
